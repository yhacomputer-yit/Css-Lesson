...
The **Playground** view extracts each lesson’s HTML and CSS into editable panels and renders the result live in a sandboxed frame. The editor and result area use a responsive split layout, with a clean device-style Result stage and hidden scrollbars that preserve touch, wheel, and keyboard scrolling. Playground controls switch the Result between Desktop (100% width), Tablet (768px), and Mobile (390px) sizes. Syntax, example, full source, and playground snippets include copy buttons, and edits are kept while switching views until Reset lesson is pressed. The Code view and editable Playground editors use a dependency-free syntax theme for HTML tags, attributes, CSS selectors, properties, strings, comments, at-rules, and values. Remote lesson images are cached, loaded lazily when they approach the preview viewport, and given a fallback state if they fail. `sitemap.xml`, `robots.txt`, canonical metadata, Open Graph tags, Twitter Card tags, and a custom `assets/og-image.png` support discovery and link sharing.
